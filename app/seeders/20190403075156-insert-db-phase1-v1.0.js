		...
		*/
		return Promise.all([
			...
			queryInterface.sequelize.query("INSERT INTO `users` (`id`, `username`, `fullname`, `password`, `sex`, `address`, `birthdate`, `phone`, `email`, `avatar`, `isActive`, `type`) VALUES" +
				...
				"(3, 'lengocha', 'Lê Ngọc Hà', '" + bcrypt.hashSync('123456', null, null).toString() + "', 'female', NULL , '1995-04-14', '0345444564', 'lnha@gmail.com', NULL, 1, 'local')," +
				"(4, 'nguoila', 'Người Lạ', '" + bcrypt.hashSync('123456', null, null).toString() + "', 'male', NULL, NULL, '0702524116', 'la@gmail.com', NULL, 1, 'local')," +
				"(5, 'lvvuong', 'Lý Văn Vương', '" + bcrypt.hashSync('123456', null, null).toString() + "', 'other', 'địa chỉ', '1997-12-24', '0802524116', 'lvvuong@gmail.com', '5-1552032846893.jpg', 1, 'local')," +
				"(7, 'nnlinh97', 'Nguyễn Ngọc Linh', '" + bcrypt.hashSync('123456', null, null).toString() + "', 'male', NULL, '1997-04-17', '0102524119', 'nnlinh@gmail.com', '7-anh avatar dep 2.jpg', 1, 'local')," +
				"(9, 'lvhai', 'Lê Văn Hải', '" + bcrypt.hashSync('123456', null, null).toString() + "', 'other', NULL, '1997-02-06', '0102521548', 'lvhai@gmail.com', '9-1552032615094.jpg', 1, 'local')," +
				"(10, 'lblong', 'Lê Bảo Long', '" + bcrypt.hashSync('123456', null, null).toString() + "', 'male', NULL, '1998-05-17', '0211546848', 'lebaolong@gmail.com', 'https://graph.facebook.com/15465484654864521000/picture?width=100', 1, 'facebook')," +
				"(11, 'ldtu', 'Lữ Đồng Tử', '" + bcrypt.hashSync('123456', null, null).toString() + "', 'male', NULL, '1990-06-17', '0315554887', 'ludongtu@gmail.com', 'https://graph.facebook.com/154654846548641500000/picture?width=100', 1, 'facebook')," +
				"(12, 'lttthao', 'Lê Thị Thanh Thảo', '" + bcrypt.hashSync('123456', null, null).toString() + "', 'female', NULL, '1989-08-24', '0315548878', 'lthaothanh@gmail.com', NULL, 1, 'local')," +
				"(13, 'DR', 'Dragon Ryuuu', '" + bcrypt.hashSync('123456', null, null).toString() + "', 'male', NULL, '1997-11-24', '0396462187', 'dragon97qn@gmail.com', NULL, 1, 'local');"),
