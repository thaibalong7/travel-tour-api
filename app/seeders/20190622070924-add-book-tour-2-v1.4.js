		...
		return queryInterface.sequelize.query("INSERT INTO `tour_turns` (`id`, `start_date`, `end_date`, `num_current_people`, `num_max_people`, `price`, `discount`, `view`, `status`, `fk_tour`, `code`, `booking_term`, `payment_term`, `isHoliday`) VALUES" +
			...
			queryInterface.sequelize.query("INSERT INTO `book_tour_contact_info` (`id`, `email`, `fullname`, `phone`, `address`, `fk_user`, `passport`) VALUES" +
				...
				return queryInterface.bulkInsert('book_tour_history', [
					...
					book_tour_history(27, await generateCode(8), formatDate(-83) + " 16:26:00", 'finished', 2, 12000000, 27, 24, 3, 4, { "name": "Ngô Thanh Hằng", "passport": "655499510", "note": "", "helper": false }),
					...
					book_tour_history(30, await generateCode(8), formatDate(-172) + " 04:17:00", 'finished', 2, 2200000, 30, 25, 3, 4, { "name": "Nguyễn Thị Mỹ Chi", "passport": "124333561", "note": "", "helper": false }),
					...
					book_tour_history(36, await generateCode(8), formatDate(-175) + " 23:05:00", 'finished', 3, 3600000, 36, 26, 3, 4, { "name": "Nguyễn Thị Hải Phượng", "passport": "261549850", "note": "", "helper": false }),
					...
					book_tour_history(44, await generateCode(8), formatDate(-107) + " 04:11:00", 'finished', 2, 2800000, 44, 28, 3, 4, { "name": "Huỳnh Hoàng Thiện", "passport": "846565465", "note": "", "helper": false }),
					book_tour_history(45, await generateCode(8), formatDate(-122) + " 04:47:00", 'finished', 2, 2800000, 45, 28, 3, 4, { "name": "Nguyễn Thị Anh Thư", "passport": "350568468", "note": "", "helper": false }),
					...
					book_tour_history(49, await generateCode(8), formatDate(-48) + " 18:45:00", 'finished', 1, 150000, 49, 30, 3, 4, { "name": "Nguyễn Hữu Phúc", "passport": "846555462", "note": "", "helper": false }),
					book_tour_history(50, await generateCode(8), formatDate(-102) + " 02:43:00", 'finished', 1, 150000, 50, 30, 3, 4, { "name": "Phạm Thị Phương Linh", "passport": "354658889", "note": "", "helper": false }),
					...
					book_tour_history(65, await generateCode(8), formatDate(-176) + " 21:31:00", 'refunded', 1, 100000, 65, 40, 3, 4, { "name": "Trương Gia Mẫn", "passport": "321435468", "note": "", "helper": false }),
